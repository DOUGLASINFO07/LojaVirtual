<?php

class Carrinho extends conexao{

    protected $total_valor, $total_peso, $itens = array();

    function GetCarrinho($sessao = NULL) {

        $i = 1;
        $subTotal = 1.00;
        $peso = 0;
        foreach ($_SESSION['PRO'] as $lista):

            $subTotal = ($lista['VALOR_US'] * $lista['QTD']);
            $this->total_valor += $subTotal;
            
            $peso = $lista['PESO'] * $lista['QTD'];
            $this->total_peso += $peso;

            $this->itens[$i] = array(
                'produto_id' => $lista['ID'],
                'produto_nome' => $lista['NOME'],
                'produto_valor' => $lista['VALOR'], // 1.000,99
                'produto_valor_us' => $lista['VALOR_US'], // 1000.99
                'produto_peso' => $lista['PESO'],
                'produto_quantidade' => $lista['QTD'],
                'produto_imagem' => $lista['IMG'],
                'produto_link' => $lista['LINK'],
                'produto_total' => $subTotal,
                'produto_subTotal' => Sistema::MoedaBR($subTotal),
                'produto_subTotal_us' => $subTotal
            );
            $i++;
        endforeach;
        if (count($this->itens) > 0) {
            return $this->itens;
        } else {
            echo '<h4 class="alert alert-danger" > Não há produtos no carrinho!</h4>';
        }
    }

    function GetTotal() {
        return $this->total_valor;
    }

    function GetPeso() {
        return $this->total_peso;
    }

    function CarrinhoADD($id) {
        $produtos = new Produtos();
        $produtos->GetProdutosID($id);
        foreach ($produtos->GetItens() as $pro) {
            $ID = $pro['produto_id'];
            $NOME = $pro['produto_nome'];
            $VALOR_US = $pro['produto_valor_us'];
            $VALOR = $pro['produto_valor'];
            $PESO = $pro['produto_peso'];
            $QTD = 1;
            $IMG = $pro['produto_imagem'];
            $LINK = Rotas::pag_ProdutosInfo() . '/' . $ID . '/' . $pro['produto_slug'];
            $ACAO = $_POST['acao'];
        }
        switch ($ACAO) {
            case 'add':
                if (!isset($_SESSION['PRO'][$ID]['ID'])) {
                    $_SESSION['PRO'][$ID]['ID'] = $ID;
                    $_SESSION['PRO'][$ID]['NOME'] = $NOME;
                    $_SESSION['PRO'][$ID]['VALOR'] = $VALOR;
                    $_SESSION['PRO'][$ID]['VALOR_US'] = $VALOR_US;
                    $_SESSION['PRO'][$ID]['PESO'] = $PESO;
                    $_SESSION['PRO'][$ID]['QTD'] = $QTD;
                    $_SESSION['PRO'][$ID]['IMG'] = $IMG;
                    $_SESSION['PRO'][$ID]['LINK'] = $LINK;
                } else {
                    //incrementa mais um item ao carrinho,
                    //Caso haja algum item inserido.
                    $_SESSION['PRO'][$ID]['QTD'] += $QTD;
                }

                echo '<h4 class="alert alert-success"> Produto Inserido! </h4>';

                break;

            case 'del':
                    $this->CarrinhoDEL($id);
                echo '<h4 class="alert alert-success"> Produto Removido! </h4>';
                break;

            case 'limpar':
                $this->CarrinhoLimpar();
                echo '<h4 class="alert alert-success"> Produtos Removidos! </h4>';
                break;
        }
    }

    private function CarrinhoDEL($id) {
        unset($_SESSION['PRO'][$id]);
    }

    private function CarrinhoLimpar() {
        unset($_SESSION['PRO']);
    }

}
